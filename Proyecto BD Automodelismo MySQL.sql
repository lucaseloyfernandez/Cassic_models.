/*  CLASSIC MODELS INC. DB.
Thanks to jovian.com and profesor Richard D Watson for let me use his DB for exercise Querys.
https://www.richardtwatson.com
https://jovian.com
Analyze by Lucas Eloy Fernandez. 

Classic Models Inc. is a distributor of small scale models of cars, motorcycles, planes, ships trains etc.
Products manufactured by Classic Models are sold in toy & gift stores around the world. 
Classic Models has offices around the world with dozens of employees. The customers of Classic Models are 
typically toy/gift stores. Each customer has a designated sales representative (an employee of Classic Models)
they interact with. Customers typically place orders requesting several products in different quantities and pay
for multiple orders at once via cheques.*/
...
INSERT INTO `offices` (`officeCode`,`city`,`phone`,`addressLine1`,`addressLine2`,`state`,`country`,`postalCode`,`territory`)
 VALUES('9','Buenos Aires','+54-91158099666','Cabildo 1234',NULL,NULL,'Arg','1636','America');

/*Explore what happens if you don't provide a value for a column marked as not null.*/
INSERT INTO `offices` (`officeCode`,`city`,`phone`,`addressLine1`,`addressLine2`,`state`,`country`,`postalCode`,`territory`)
 VALUES('10','Entre Rios',NULL,'Sarmiento 1234',NULL,NULL,'Arg','1450','America');
 
/*Try adding an entry with a primary key matching the an existing entry.*/
INSERT INTO `offices` (`officeCode`,`city`,`phone`,`addressLine1`,`addressLine2`,`state`,`country`,`postalCode`,`territory`)
 VALUES('1','Buenos Aires','+54-91158099666','Juramento 1234',NULL,NULL,'Arg','1636','America');
...
INSERT INTO `employees` VALUES
(1669,'Fernandez','Lucas','x101','lucasDBmanager.com','1',NULL,'Data Base Manager');
SELECT*
FROM employees
WHERE employeeNumber = 1669;

/*EXERCISES:
Try inserting an entry into employees with an invalid value for officeCode.*/
INSERT INTO `employees` VALUES
(1890,'MArquees','Rigoberto','x101','RigoDBmanager.com',NULL,NULL,'Data Base Manager');

/*Try inserting an entry into employees with an invalid value for reportsTo.*/
INSERT INTO `employees` VALUES
(1990,'Marquez','Marc','x101','MarcDBmanager.com',"2",noone,'Data Base Manager');

/*Create a new office location and add some employees for the new location.*/
INSERT INTO `offices` 
(`officeCode`,`city`,`phone`,`addressLine1`,`addressLine2`,`state`,`country`,`postalCode`,`territory`) VALUES 
('12','Modena','+34 999 219 4782','Rigori Strada','NULL','Fiorenza','Italiy','94080','NA');
INSERT INTO `employees` VALUES
(1990,'Rossi','Valentino','x046','ValenDBmanager.com',"12",NULL,'Data Base Manager');
INSERT INTO `employees` VALUES
(1992,'Bagnania','Francesco','x063','PecoDBmanager.com',"12",NULL,'Data Base Manager');
SELECT *
FROM employees
WHERE officeCode = "12";
...
SELECT COUNT(productCode) as productsQuantity , productLine
FROM products
...
/*What are the names of executives with VP or Manager in their title? Use the CONCAT function to combine the employee's first name and last name
...
SELECT orderNumber, FORMAT(SUM(quantityOrdered*priceEach),2) AS `Order value`
FROM OrderDetails
...
SELECT customerName, CONCAT(firstName," ", lastName) AS employeeRepresentative
...
SELECT customers.customerName, FORMAT(SUM(payments.amount),2)
FROM customers JOIN payments
	ON customers.customerNumber = payments.customerNumber
    GROUP BY customerName;

/*How many orders have been placed by Herkku Gifts?*/
SELECT COUNT(orders.orderNumber) as numberOfOrders, customerName
FROM orders JOIN customers
ON orders.customerNumber = customers.customerNumber
GROUP BY customers.customerName HAVING customerName = "Herkku Gifts";

/*Who are the employees in Boston?*/
SELECT employees.employeeNumber,CONCAT( employees.lastName, " ", employees.firstName) AS employee, offices.city
...
SELECT customers.customerName, CONCAT("$", FORMAT(amount, 2)) as payments
FROM customers JOIN payments
ON customers.customerNumber = payments.customerNumber
...
SELECT customerName, COUNT(orders.status) as onHoldStatus
FROM orders JOIN customers
ON  orders.customerNumber = customers.customerNumber
WHERE status = "On Hold"
GROUP BY customerName;
...
SELECT customers.customerName, Orders.orderNumber, FORMAT(SUM(orderDetails.quantityOrdered * orderDetails.priceEach),2) AS Total
FROM Customers JOIN Orders
ON Customers.customerNumber = Orders.customerNumber
JOIN OrderDetails ON Orders.orderNumber = OrderDetails.orderNumber
GROUP BY orders.orderNumber HAVING SUM(orderDetails.quantityOrdered * orderDetails.priceEach) > 25000;
...
SELECT products.productName, orders.status, FORMAT(products.quantityInStock,0) AS "Quantity in stock"
FROM orders JOIN orderdetails
	ON orders.orderNumber = orderDetails.orderNumber
    JOIN products 
    ON products.productCode = orderDetails.productCode
...
SELECT *
FROM Products 
WHERE LOWER(productName) REGEXP "ship|boat";
...
WHERE jobTitle = 'Sales Representative' AND officeCode = "7";
...
ALTER TABLE orders ADD orderDetails text;
/*Changing the data type of a column: */
ALTER TABLE orders MODIFY COLUMN orderDetails varchar(50);
/*Renaming a column:*/
ALTER TABLE orders RENAME COLUMN orderDetails TO orderDemands;
/*Removing a column from a table:*/
ALTER TABLE orders DROP COLUMN orderDemands;
...
/*EXERCISE: Retrieve details of all the customers in the United States who have made payments between 
April 1st 2003 and March 31st 2004.*/
SELECT *
FROM customers
WHERE country = "USA"  in (SELECT distinct customerNumber
FROM payments
...
SELECT productCode, SUM(quantityOrdered) AS cantidad
FROM orderdetails
group by productCode;

/*QUESTION: Find the total no. of payments and total payment amount for each customer for payments made
 before October 28, 2004.*/
 SELECT COUNT(checkNumber) AS n°payments, SUM(amount) totalAmount, customerNumber
 FROM payments
 WHERE paymentDate < "2004-10-28"
 GROUP BY customerNumber;
 
/*EXERCISE: Modify the above query to also show the minimum, maximum and average payment value for each customer.*/
SELECT MIN(amount), MAX(amount), AVG(amount), customerNumber
FROM payments
GROUP BY customerNumber;
...
ORDER BY  customerName; 
 ...
 /*QUESTION: Display the list of the 5 most expensive products in the "Motorcycles" product line with their 
 price (MSRP) rounded to dollars.*/
 
 SELECT productName, ROUND(MSRP) as mostExpensive
 FROM products
...
((MSRP - buyPrice)*100/buyPrice) for the 10 products with the highest profit margin. Round the profit margin 
to 2 decimals.*/

SELECT productCode, productName, buyPrice, MSRP, ROUND(((MSRP - buyPrice)*100/buyPrice), 2) AS profitMargin
FROM products
ORDER BY profitMargin DESC
LIMIT 10;

 /*QUESTION: List the largest single payment done by every customer in the year 2004, ordered by the 
 transaction value (highest to lowest).*/
 
SELECT MAX(amount) AS maxPayment, customerNumber
FROM payments
...
/*QUESTION: For the above query, format the amount properly with a dollar symbol and comma separation
 (e.g $26,267.62), and also show the month as a string.*/
 SELECT YEAR(paymentDate) as `year`, 
	DATE_FORMAT(paymentDate, "%M") AS `month`, 
	CONCAT("$", FORMAT(SUM(amount), 2)) AS `totalPayments`
    FROM payments 
    GROUP BY `year`, MONTH(paymentDate), `month` /* MONTH(paymentDate) lo usamos solo en el group by y order by solo para organizar los meses
    por orden y no alfabeticamente. */
    ...
    SELECT employees.employeeNumber, employees.lastName , employees.firstName, offices.phone, offices.addressLine1, offices.addressLine2
    FROM employees JOIN offices
    ON employees.officeCode = offices.officeCode;
...
SELECT CONCAT(employees.firstName, ' ',employees.lastName) AS Name, 
	FORMAT(0.05 * SUM(orderDetails.quantityOrdered * orderdetails.priceEach),0) AS Commission
...
SELECT DATEDIFF(MAX(orderDate), MIN(orderDate)) AS dateDiffernce
FROM orders;

/*Compute the average time between order date and ship date for each customer ordered by the largest difference.*/
SELECT AVG(DATEDIFF(shippedDate,orderDate)) AS avgTime, customerName
FROM orders JOIN customers
ON orders.customerNumber = customers.customerNumber
...
SELECT FORMAT(SUM(priceEach*quantityOrdered),0) as OrderValues
FROM orderDetails JOIN orders
ON orderDetails.orderNumber = orders.orderNumber
WHERE MONTH(orderDate)=8 and YEAR(orderDate)=2004;

/*......VIEWS...........

Compute the total value ordered, total amount paid, and their difference for each customer for orders placed in 2004
 and payments received in 2004. Only report those customers where the absolute value of the difference is greater than $100. 
...
/*............SAME QUERY WITH TEMPORARY TABLES..............*/
...
/*List the employees who report to those employees who report to Diane Murphy. Use the CONCAT function to combine
...
WITH
t1 AS (SELECT MONTH(paymentDate) AS period, sum(amount) AS payments 
FROM Payments
WHERE YEAR(paymentDate) = 2004
GROUP BY MONTH(paymentDate)),
t2 AS (SELECT MONTH(orderDate) AS period, sum(quantityOrdered*priceEach) AS orders 
FROM Orders JOIN OrderDetails
ON Orders.orderNumber = OrderDetails.orderNumber
WHERE YEAR(orderDate) = 2004
GROUP BY MONTH(orderDate))
SELECT t1.period, FORMAT(payments/orders,2) AS ratio 
FROM t1 JOIN t2
ON t1.period = t2.period;

/*Report the total payments by date*/
SELECT CONCAT("$",FORMAT(SUM(amount),2)) as totalPayments, COUNT(amount) as numberOfPayments, paymentDate
FROM payments
GROUP BY paymentDate;

/*List the amount paid by each customer.*/
SELECT CONCAT("$",FORMAT(SUM(amount),2)) as totalAmount, customers.customerName
FROM payments JOIN customers
ON payments.customerNumber = customers.customerNumber
...
CASE
	WHEN amount BETWEEN 0 AND 50000 THEN "regular payer"
    WHEN amount BETWEEN 50000  AND 100000 THEN "big payer"
	WHEN amount > 100000 THEN "SUPER payer"
END as payments
FROM customers JOIN payments
ON customers.customerNumber = payments.customerNumber;
